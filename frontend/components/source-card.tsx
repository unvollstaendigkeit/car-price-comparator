...
export function SourceCard({ sourceKey, data }: { sourceKey: string; data: SourceResult }) {
  ...
  // shown — a thin sample is surfaced with a caution banner, never hidden.
      ...
      {hasRetrievalError ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 py-10 text-center">
          ...
          <p className="max-w-sm text-sm text-muted">
            No comparable listings were found for this exact spec on {meta.name}. This is an absence of matching
            cars, not a fetch failure.
          </p>
        </div>
      ) : (
        <>
          ...
          {isThinSample ? (
            <p className="border-b border-border bg-caution/10 px-4 py-2 text-xs text-caution">
              Thin sample (n={data.comparable_count}) — shown for reference, but too few for a reliable market
              price for this exact spec.{data.sample_warning ? ` ${data.sample_warning}` : ""}
            </p>
          ) : (
            data.sample_warning && (
              <p className="border-b border-border bg-caution/10 px-4 py-2 text-xs text-caution">
